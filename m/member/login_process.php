...
switch($mode){
	...
	case "logins2"		:	function_logins2();		break;
...
function function_naver_ins(){
	...
	if(mysql_num_rows($res) > 0) {
		...
		if(strlen(trim($row2['tel']))<=5){			
			...
			//https://www.phpschool.com/gnuboard4/bbs/board.php?bo_table=qna_function&wr_id=375547 참고
...
function function_logout(){
	...
	echo "<script>alert('로그아웃이 되었습니다.'); top.location.href='/m/'; </script>";
...
function function_logins2(){
	...
	$total = mysqli_num_rows();
	$row = mysql_fetch_assoc($cdata);
		
	if($total==1){
		if($row['grade']=="i"){
			$json_data = array("result"=>true,'status'=>'2','data'=>$data);
			/*
			$url = "/m/member/email_ck.php";
		...
		$data['reg_dv'] = $row['reg_dv'];

		$json_data = array("result"=>true,'status'=>'0','data'=>$data);
	} else {
		/* 로그인 실패*/
		
		$json_data = array("result"=>true,'status'=>'1','data'=>$sql);
	}
	echo json_encode($json_data);
	dbClose($dbc);
	exit;
