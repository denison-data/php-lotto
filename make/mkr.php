<?
/*매주 수요일 오전 10시 20분에 번호 발생 -> /cron/mkr.php 로 이전 */
...
while($mrow = mysql_fetch_assoc($mem_res)){
	...
	$grow = mysqli_fetch_array($qdata);

	if($totals>0){
		switch($grow['g_grade']){
			case "n"	:	$mcount = "1"; break;
			case "e"	:	$mcount = "1"; break; //이벤트 1개월
			case "c"	:	if($grow['g_code']=="CD2"){ $mcount = "2";} else {$mcount = "1";} break; //쿠폰 1개월
			case "s"	:	$mcount = "2"; break;
			case "g"	:	$mcount = "2"; break;
			case "j"	:	$mcount = "2"; break;
			case "p"	:	$mcount = "4"; break;
			case "m"	:	$mcount = "2"; break;
			case "a"	:	$mcount = "2"; break; //한달골드
		}
		$mem_que = "
		...
		/* 실버회원중 01024758425 요 회원 10개 요청*/
		if($mrow['code']=="YYquJ" || $mrow['code']=="6zL1H"  || $mrow['code']=="wjBg0" ||$mrow['code']=="K1OHh" || $mrow['code']=="0U3KR" || $mrow['code']=="6RhAy" ){
			$mcount = "1";
		}
		if( $mrow['code']=="7Qi9V" || $mrow['code']=="CG1u4" || $mrow['code']=="MoSWU" || $mrow['code']=="rnOAs" || $mrow['code']=="aHztx"){
			$mcount = "2";
		}
		if($mrow['code']=="pQpxg" || $mrow['code']=="N3ZwQ" || $mrow['code']=="73xVk" || $mrow['code']=="IcdqU"){
			$mcount = "3";
		}
		/*
		if($mrow['code']=="rnOAs"){
...
function res_db_plus($fnum, $ep, $mcode,$sel){
	...
	//echo $total_count;
	$que_ct = "Select * from {$tb} Where code in ('$mcode') And seq in ('$next_seq') And type not in ('A')";
...
function res_db($code, $mcount, $mgrade){
	...
	$que_ct = "Select * from {$tb} Where code in ('$code') And seq in ('$next_seq') And type not in ('A')";
	$q_res = dbQuery($que_ct, $dbc);
	$total = mysqli_num_rows();
	...
	if($total==0){
		...
		dbQuery($que, $dbc);
		//if($mgrade=="n"){
		//	$max = 10; //등급에 따라 나눔 n== 5 
		//} else {
			$max = 10*$mcount;
		//}	
		$ct_qry = "Select Count(*) from $tb Where 1=1 And seq in ('$next_seq') And type in ('R') And code in ('$code')";
		$res = dbQuery($ct_qry);
