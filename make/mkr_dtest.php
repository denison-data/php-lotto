...
while($mrow = mysql_fetch_assoc($mem_res)){
	...
	if($totals>0){
		...
		if($mrow['code']=="YYquJ" || $mrow['code']=="6zL1H"  || $mrow['code']=="wjBg0" ||$mrow['code']=="K1OHh" || $mrow['code']=="0U3KR" ){
			$mcount = "1";
		}
		if( $mrow['code']=="7Qi9V" || $mrow['code']=="CG1u4" || $mrow['code']=="MoSWU" || $mrow['code']=="rnOAs" || $mrow['code']=="6RhAy" || $mrow['code']=="aHztx"){
			$mcount = "2";
		}
...
function res_db_plus($fnum, $ep, $mcode,$sel){
	...
	//echo $total_count;
	$que_ct = "Select * from {$tb}_t Where code in ('$mcode') And seq in ('$next_seq') And type not in ('A')";
...
function res_db($code, $mcount, $mgrade){
	...
	$que_ct = "Select * from {$tb}_t Where code in ('$code') And seq in ('$next_seq') And type not in ('A')";
	$q_res = dbQuery($que_ct, $dbc);
	$total = mysqli_num_rows();
	if($total==0){
		$_BALL = array();
